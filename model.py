...
class FPAv2(nn.Module):
    ...
    def forward(self, x):
        # x shape: 512, 16, 16  torch.Size([batch_size, 512, 16, 100]
        # print(x.shape)
        x_glob = self.glob(x)  # 256, 1, 1  ->[1, 256, 1, 16]
        # print(x_glob.shape)
        x_glob = F.upsample(x_glob, scale_factor=(16, 4), mode='bilinear', align_corners=True)  # 256, 16, 16
        # print(x_glob.shape)
        ...
        x = x * d2
        # print(x.shape) # [batch_size, 256, 16, 100]
        # print(x.shape, x_glob.shape)
        x = x + x_glob
...
class Decoderv2(nn.Module):
    ...
    def forward(self, up_p, x_p):
        ...
        :param up_p: need to up (w,h)->(2*w,2*h)
        :param x_p: need to down (w,h)->(w/2,h/2)
        :return:
        '''
...
class Res34Unetv4(nn.Module):
    def __init__(self):
        ...
        # RGB ->Gray
        weight_rgb = self.resnet.conv1.weight
        ...
        self.resnet.conv1.weight = torch.nn.Parameter(weight_grey)
        self.resnet.conv1.bias = bias
###################################
        ...
        self.logit = nn.Sequential(nn.Conv2d(320, 64, kernel_size=3, padding=1),
                                   nn.ELU(True),
                                   nn.Conv2d(64, 4, kernel_size=1, bias=False))
    def forward(self, x):
        ...
        d5 = self.decode5(f, e5)  # 64, 16, 16 yyh change --> channel 32  16 16
        # print(d5.shape) torch.Size([2, 32, 16, 100])
        ...
        f = torch.cat((d1,
                       ...
                       F.upsample(d5, scale_factor=16, mode='bilinear', align_corners=True)), 1)  # 320, 256, 1600/256
        # print(f.shape) torch.Size([batch_size, 160, 256, 1600 or 256])
        f = F.dropout2d(f, p=0.3)
        #
...
###################################
        ...
        self.logit = nn.Sequential(nn.Conv2d(320, 64, kernel_size=3, padding=1),
                                   nn.ELU(True),
                                   nn.Conv2d(64, 4, kernel_size=1, bias=False))
        self.clf = nn.Sequential(nn.Conv2d(64, 4, kernel_size=1, bias=False))
        # self.resnet.layer5
    def forward(self, x):
        # x: (batch_size, 3, 256, 256)
        ...
        #
        cls = self.clf(d5)
        cls = cls.view((batch_size, 4, -1))
        cls = torch.mean(cls, dim=2) # torch.Size([1, 5])
        # print(cls.shape) # torch.Size([4, 5, 16, 16])
        logit = self.logit(f)  # 4, 256, 256
